...
| Text search | Substring match on headword and entry text across the full corpus (no result cap); session-generated entries included via client-side text match; typeahead dropdown shows prefix matches (corpus + session) as you type — accent-normalised so clicking "E" also returns É/È entries; fuzzy suggestions (Levenshtein) appear when text search returns nothing |
