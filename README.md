...
| Text search | Substring match on headword and entry text; session-generated entries included in results via client-side text match; typeahead dropdown shows prefix matches (corpus + session) as you type; fuzzy suggestions (Levenshtein) appear when text search returns nothing |
