...
| Text search | Substring match on headword and entry text across the full corpus (no result cap); session-generated entries included via client-side text match; typeahead dropdown shows prefix matches (corpus + session) as you type; fuzzy suggestions (Levenshtein) appear when text search returns nothing |
